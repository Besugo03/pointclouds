...
def load_and_downsample_point_cloud_new(
    file_path, target_points=30000, max_iter=10, tolerance=0.1
):
    print("Loading point cloud...")
    points = np.loadtxt(file_path, delimiter=",")
    print(f"Loaded {points.shape[0]} points.")
    ...
    for _ in range(max_iter):
        ...
        if abs(num_points - target_points) / target_points <= tolerance:
            print(
                f"Found optimal voxel size: {current_voxel:.5f} → {num_points} points"
            )
    ...
    print(
        f"Using best voxel size: {best_voxel:.5f} → {best_num_points} points (target: {target_points})"
    )
    downsampled = pcl.voxel_down_sample(best_voxel)
    return np.asarray(downsampled.points)


def load_and_downsample_point_cloud_by_skip(
    surfaces: list, surfacePoints: list, step_size: int
):
    ...
    for i in range(len(surfacePoints)):
        ...
        surfacePoints[i] = [
            surfacePoints[i][j]
            for j in range(len(surfacePoints[i]))
            if j % step_size == 0
        ]
    ...
    for pointsList in surfacePoints:
        ...
        points = np.array(
            [list(map(float, point[0].split(","))) for point in pointsList]
        )
...
def calculate_normals_vectorized(points, k_neighbors=K_NEIGHBORS):
    ...
    for i in range(len(points)):
        ...
        normals[i] = eigenvectors[
            :, 0
        ]  # eigh sorts eigenvalues in ascending order, so last index is smallest
...
def calculate_curvature(points, KDtree, normals, k_neighbors=K_NEIGHBORS):
    ...
    for i in range(len(points)):
        ...
        angles = [
            np.arccos(np.clip(np.dot(normals[i], n), -1.0, 1.0)) for n in local_normals
        ]
...
def calculate_curvature_vectorized(points, normals, tree, k_neighbors=K_NEIGHBORS):
    ...
    dot_products = np.sum(
        main_normals * neighbor_normals, axis=2
    )  # shape: (n_points, k_neighbors)
...
def visualize_clusters(points, labels, normals):
    try:
        ...
        point_colors = np.array(
            [colors[label] if label != -1 else [0, 0, 0] for label in labels]
        )
...
def calculate_and_export_pointcloud(
    surfacepoints: list,
    surfaces: list,
    input_file_path: str,
    output_file_dir: str,
    ...
    # load and downsample the point cloud
    points = load_and_downsample_point_cloud_by_skip(
        surfaces, surfacepoints, step_size=10
    )
    print("LastPoints :")
    print(lastpoints)
    print("LastPointsPosition :")
    ...
    with open(output_file_path, "w") as f:
        ...
        for i in range(len(points)):
            f.write(
                f"{points[i][0]},{points[i][1]},{points[i][2]},{normals[i][0]},{normals[i][1]},{normals[i][2]},{curvatures[i]}\n"
            )
            if i in lastpointsPosition:
                print(
                    "i in lastpoints position : ",
                    surfaces[lastpointsPosition.index(i) + 1],
                )
